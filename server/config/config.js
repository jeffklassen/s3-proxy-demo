...
config.s3 = function buildAWSS3() {
    ...
    if (!process.env.REGION || !process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY || !process.env.S3_HOST || !process.env.BUCKET) {
        console.log("You must enter an AWS region, aws_access_key_id, and aws_secret_access_key");
        console.log("Usage: REGION=us-east-1 AWS_ACCESS_KEY_ID=akid AWS_SECRET_ACCESS_KEY=asak BUCKET=bucket_name S3_HOST=hostname npm start");
        process.exit(1);
    }
